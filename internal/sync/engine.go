...
func (e *Engine) classifyProviderChangedPath(
	...
	for _, agentType := range agents {
		...
		if !e.providerProcessEnabled(agentType) {
			continue
		}
...
func (e *Engine) discoverProviderSources(
	...
	for _, agentType := range agents {
		...
		if e.providerMigrationModes[def.Type] !=
...
// processProviderFile owns parsing for provider-authoritative agents. A resolved
// provider source is required; ambiguous changed-path lookups report a provider
// lookup error instead of falling back to legacy parsing.
