...
func (s JSONLSourceSet) FindSource(
	...
	if req.StoredFilePath != "" {
		if source, ok := s.sourceForPath(req.StoredFilePath); ok {
...
func (s JSONLSourceSet) sourceFileInfo(path string) (os.FileInfo, bool) {
...
func (s JSONLSourceSet) pathFromSource(source SourceRef) (string, bool) {
