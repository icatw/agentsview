...
func NewSiblingMetadataSourceSet(
	...
	return SiblingMetadataSourceSet{
		JSONLSourceSet: NewJSONLSourceSet(provider, roots, options),
...
func (s SiblingMetadataSourceSet) SourcesForChangedPath(
	...
	for _, root := range s.roots {
		...
		source, ok := s.sourceForPath(sourcePath)
		if ok {
...
func (s SiblingMetadataSourceSet) Fingerprint(
	...
	resolved, ok := s.sourceFromRef(source)
	if !ok {
...
func (s SiblingMetadataSourceSet) sourceFromRef(source SourceRef) (SourceRef, bool) {
