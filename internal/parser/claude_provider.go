...
func (p *claudeProvider) Parse(
	...
	results, excludedIDs, err := ParseClaudeSessionWithExclusions(path, project, machine)
	if err != nil {
		return ParseOutcome{}, err
	}
...
func (s claudeSourceSet) Discover(ctx context.Context) ([]SourceRef, error) {
	...
	for _, root := range s.roots {
		...
		for _, file := range DiscoverClaudeProjects(root) {
...
func (s claudeSourceSet) FindSource(
	...
	for _, root := range s.roots {
		path := FindClaudeSourceFile(root, req.RawSessionID)
		if path == "" {
			continue
		}
