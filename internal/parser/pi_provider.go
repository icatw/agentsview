...
func (f piProviderFactory) NewProvider(cfg ProviderConfig) Provider {
	...
	return &piProvider{
		...
		sources: newPiSourceSet(cfg.Roots),
	}
}
...
func (p *piProvider) sourceForSessionID(
	...
	for _, entry := range entries {
		...
		source, ok := p.sources.sourceForPath(candidate)
		if ok {
			return source, true, nil
		}
	}
...
func (p *piProvider) Parse(
	...
	sess, msgs, err := ParsePiSession(path, "", machine)
	if err != nil {
		return ParseOutcome{}, err
	}
	if sess == nil {
...
func newPiSourceSet(roots []string) DirectoryJSONLSourceSet {
