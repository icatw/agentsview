...
func (p *antigravityProvider) Parse(
	...
	sess, msgs, usageEvents, err := ParseAntigravitySession(
		src.Path,
...
func (s antigravitySourceSet) Discover(ctx context.Context) ([]SourceRef, error) {
	...
	for _, root := range s.roots {
		...
		for _, file := range DiscoverAntigravitySessions(root) {
...
func (s antigravitySourceSet) FindSource(
	...
	for _, root := range s.roots {
		path := FindAntigravitySourceFile(root, req.RawSessionID)
		if path == "" {
			continue
		}
