...
func TestWorkBuddyProviderParseMainAndSubagent(t *testing.T) {
	...
	mainOutcome, err := provider.Parse(context.Background(), ParseRequest{
		Source:      sources[0],
		Fingerprint: SourceFingerprint{Key: sourcePath, Hash: "main-hash"},
	})
	require.NoError(t, err)
	...
	assert.Equal(t, "main-hash", mainResult.Result.Session.File.Hash)
	...
	subOutcome, err := provider.Parse(context.Background(), ParseRequest{
		Source:      sources[1],
		Fingerprint: SourceFingerprint{Key: subagentPath, Hash: "sub-hash"},
	})
	require.NoError(t, err)
	...
	assert.Equal(t, "sub-hash", subResult.Result.Session.File.Hash)
}
